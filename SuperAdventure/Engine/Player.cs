...
namespace Engine
{
    public class Player : LivingCreature
    {
        ...
        public bool HasRequiredItemToEnterThisLocation(Location location)
        {
            ...
            {
                //there is no required item for this location, so "true"
                return true;
            }

            //see if the player has the item in the inventory
            ...
            //We didn't find the required item in their inventory, so "false"
            return false;
        }
            ...
            foreach (QuestCompletionItem qci in quest.QuestCompletionItems)
            {
                ...
                foreach (InventoryItem ii in Inventory)
                {
                    if (ii.Details.ID == qci.Details.ID) //the player has the item in their inventory
                    {
                        ...
                        if (ii.Quantity < qci.Quantity) //the player does not have enough of this item
                        {
                            return false;
                        }
                    }
                ...
                if (!foundItemInPlayersInventory) // if it is false
                {
                    return false;
                }
            }
            //if we got here, then the player has all the required items, and enough quantity.
            return true;
        }
        ...
        public void MarkQuestCompleted(Quest quest)
        {
            foreach (PlayerQuest pq in Quests)
            {
                if (pq.Details.ID == quest.ID)
                {
                    ...
                    return; //we found the quest, marked it complete, get out of this function
                }
            }
        }
        ...
        public string ToXmlString()
        {
            ...
            //Create the "PlayerQuests" node to hold each PlayerQuest node
